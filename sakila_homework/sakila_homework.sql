USE sakila;
-- 1a. Display the firt and last names of all actors from the table 'actor'
...
-- whom you know only the first name, "Joe." What is one query you would use
...
-- 2c Find all actors whose last names contain the letter 'LI'. This time, order 
...
-- last name
SELECT last_name, COUNT(*) AS '# Same Last Name'
...
SELECT last_name, COUNT(*) AS '# >=2 Same Last Name'
FROM actor GROUP BY last_name
HAVING count(*) >=2;	

-- 4c. The actor 'Harpo Williams' was accidentally entered in the 'actor' table
-- as 'Groucho Williams'. Write a query to change the record
UPDATE actor
SET first_name = 'HARPO'
WHERE first_name = 'GROUCHO' AND last_name = 'WILLIAMS';

-- 4d. Change the first name of the actor 'HARPO' to 'GROUCHO'
UPDATE actor
...
SELECT f.title AS 'Film Title', COUNT(fa.actor_id) 
AS 'Number of Actors'
FROM film_actor fa
INNER JOIN film f
...
-- list the total paid by each customer. List the customers alphabetically by 
-- last name

SELECT c.last_name AS 'last_name',sum(p.amount) AS 'Total Paid'
FROM customer c
JOIN payment p
...
-- 7f. Write a query to display how much business, in dollars for each store
...
-- 7g. Write a query to display for each store its store ID, city, country.
...
-- you may need to use the following tables: category, film_category, 
